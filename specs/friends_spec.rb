...
class TestFriends < MiniTest::Test
  ...
  def test_getting_favourite_tv_show
    result_tv = favourite_tv_show(@person2)
    assert_equal("Baywatch", result_tv)
  end
...
def test_food_preference
  result_food = likes_to_eat(@person4, "spaghetti")
...
def test_how_many_friends
  ...
  def test_total_monies
    result_total = total_monies(@people)
    assert_equal(143, result_total)
  end

  # 7. For two given people, allow the first person to loan a given value of money to the other
  # (hint: our function will probably need 3 arguments passed to it... the lender, the lendee, and the amount for this function)
  # (hint2: You should test if both the lender's and the lendee's money have changed, maybe two assertions?)
  def test_friendly_loan_shark
    friendly_loan_transaction(@person4, @person1, 1)
    assert_equal(19, @person4[:monies])
